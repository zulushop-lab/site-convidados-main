...
|---|---|---|---|
| M-1 | RT-1: cliente nunca cria contribuição com status diferente de `pending` | `rg -n "status in \['pending'\s*,\s*'completed'\]" firestore.rules` E `rg -n "data\.status == 'pending'" firestore.rules` | 1ª busca retorna 0 linhas E 2ª busca retorna >=1 linha |
| M-2 | RT-2: teto de valor aplicado em `isValidContribution` | `rg -n "data\.amount > 0 && data\.amount <= 100000" firestore.rules` | retorna >=1 linha |
| M-3 | RT-3/RT-4: `keys().hasOnly([...])` + `donorEmail` obrigatório em contributions | `rg -n "hasOnly" firestore.rules` E `rg -n "data\.donorEmail is string" firestore.rules` | ambas as buscas retornam >=1 linha cada |
| M-4 | RT-5: `allow update` em contributions restrito a admin com transição de status válida | `rg -n "allow update: if isAdmin\(\)" firestore.rules` E `rg -n "isValidStatusTransition" firestore.rules` | ambas retornam >=1 linha; helper `isValidStatusTransition` definido no arquivo |
| M-5 | Critério de aceite (rules-unit-testing) contributions: `status:'completed'`/`'processing'`/`'failed'` anônimo, `amount` fora de faixa, sem `donorEmail`, campo extra → todos `assertFails`; `status:'pending'` válido → `assertSucceeds`; update admin com transição válida `assertSucceeds`, update cliente e transição inválida (`completed→pending`) `assertFails` | teste `@firebase/rules-unit-testing` no Firestore Emulator cobrindo todos os casos acima | suíte de testes de rules de contributions sai com exit code 0 (todos os asserts PASS) |
| M-6 | RT-7/RT-8: `rsvps` com `hasOnly` do superset e caps em `adults`/`childrenCount`/`dietary`/`message` | `rg -n "hasOnly\(\['adults'" firestore.rules` E `rg -n "data\.adults\.size\(\) <= 20" firestore.rules` E `rg -n "size\(\) <= 1000" firestore.rules` | as três buscas retornam >=1 linha cada |
| M-7 | Critério de aceite (rules-unit-testing) rsvps: campo fora do `hasOnly`, `adults.size()>20`, `dietary`/`message`>1000 chars → `assertFails`; payload válido com `familyId`/`guestId` opcionais → `assertSucceeds` | teste `@firebase/rules-unit-testing` no emulator cobrindo os casos acima | suíte de testes de rules de rsvps sai com exit code 0 |
| M-8 | RT-9: decisão sobre `isSignedIn()` registrada e consistente | `rg -n "isSignedIn\(\)" firestore.rules` E `rg -n "SPEC-RSVP-AUTH" firestore.rules` | se `isSignedIn()` aparece ativo (não comentado) em create de contributions e rsvps → PASS; se comentado, há TODO referenciando `SPEC-RSVP-AUTH` no mesmo arquivo (>=1 linha) → PASS |
| M-9 | RT-10: `firebase.json` declara `firestore.rules` como alvo de deploy | `test -f firebase.json && rg -n "\"rules\":\s*\"firestore\.rules\"" firebase.json` | arquivo existe E busca retorna >=1 linha |
| M-10 | RT-11: `DRAFT_firestore.rules` removido do repo | `test ! -e DRAFT_firestore.rules; echo $?` | exit code 0 (arquivo não existe) |
| M-11 | RT-12: higiene de segredos — config real ignorada, exemplo versionado | `git check-ignore firebase-applet-config.json` E `test -f firebase-applet-config.example.json` E `git ls-files --error-unmatch firebase-applet-config.example.json` | `check-ignore` imprime o caminho (exit 0); exemplo existe e está versionado (exit 0) |
| M-12 | RT-14: `lib/firestore-errors.ts` não loga/propaga PII e não serializa `Error` com JSON de PII | `rg -n "email\|providerInfo\|tenantId" lib/firestore-errors.ts` (no objeto logado/propagado) E `rg -n "throw new Error\(JSON\.stringify" lib/firestore-errors.ts` | nenhuma referência a `email`/`providerInfo`/`tenantId` no payload logado/lançado E 2ª busca retorna 0 linhas |
| M-13 | Critério de aceite final: build do projeto passa após a refatoração | `npm run build` | exit code 0 |
| M-14 | [humano] Critério de aceite Error layer: ao provocar erro de permissão, nenhum email/PII aparece no console do navegador nem na UI; e PREFLIGHT (seção 9) confirmou banco named vs default antes do deploy | inspeção visual do console/UI durante repro de `permission-denied` + confirmação no Firebase Console do banco correto | verificação humana registrada no PR (sem PII observada; banco confirmado) |

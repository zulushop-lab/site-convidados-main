...
|---|---|---|---|
| M-1 | RT-3 / aceite: `code` resolvido via `await params` e nome `'Rafael'` removido | `rg -n "await params" app/rsvp/\[code\]/page.tsx` **e** `rg -n "'Rafael'" app/rsvp/\[code\]/page.tsx` | 1ª contagem ≥1 **e** 2ª contagem = 0 |
| M-2 | RT-1: Anonymous Auth + helper idempotente em `lib/firebase.ts` | `rg -n "signInAnonymously" lib/firebase.ts` **e** `rg -n "ensureAnonymousAuth" lib/firebase.ts` | ambas as contagens ≥1 |
| M-3 | RT-2: AuthContext existe e expõe `uid`/`isAdmin` | `test -f lib/context/AuthContext.tsx && rg -n "onAuthStateChanged" lib/context/AuthContext.tsx && rg -n "isAdmin" lib/context/AuthContext.tsx` | arquivo existe **e** ambas as contagens ≥1 |
| M-4 | RT-4/RT-6: GuestContext existe e persiste identidade em `sessionStorage` na chave versionada | `test -f lib/context/GuestContext.tsx && rg -n "guest\.identity\.v1" lib/` | arquivo existe **e** contagem ≥1 |
| M-5 | RT-7 / aceite: RSVP por família usa `setDoc(doc(db,'rsvps',familyId))` e elimina `addDoc` em `rsvps` | `rg -n "setDoc\(doc\(db, *'rsvps', *familyId" app/` **e** `rg -n "addDoc\(.*rsvps" app/presenca/page.tsx` | 1ª contagem ≥1 **e** 2ª contagem = 0 |
| M-6 | RT-7: payload de RSVP carimba identidade e timestamps de servidor | `rg -n "confirmedBy" app/` **e** `rg -n "updatedBy" app/` **e** `rg -n "serverTimestamp" app/presenca/page.tsx` | as três contagens ≥1 |
| M-7 | RT-9: lookup via índice `codes/{code}` por ID (sem `list`/`where('code','==')` no cliente) | `rg -n "doc\(db, *'codes'" app/` **e** `rg -n "where\('code', *'=='" app/` | 1ª contagem ≥1 **e** 2ª contagem = 0 |
| M-8 | RT-11 / aceite: `contributions` carimbado com `familyId`/`guestId` (campos de identidade) | `rg -n "familyId" app/presentes/checkout/page.tsx` **e** `rg -n "guestId" app/presentes/checkout/page.tsx` | ambas as contagens ≥1 |
| M-9 | RT-10 / aceite: regras para `codes`/`families`/`guests` adicionadas a `firestore.rules` | `rg -n "match /codes/\{code\}" firestore.rules` **e** `rg -n "match /families/\{familyId\}" firestore.rules` **e** `rg -n "match /guests/\{guestId\}" firestore.rules` | as três contagens ≥1 |
| M-10 | RT-10 / aceite: `create` de `rsvps` exige autenticação e `isValidRSVP` exige `familyId` | `rg -n "isSignedIn" firestore.rules` **e** `rg -n "familyId" firestore.rules` | ambas as contagens ≥1 |
| M-11 | RT-10: rule de unit-testing — `list` em `families` por não-admin é negado e `get codes/<code>` é permitido | teste `@firebase/rules-unit-testing`: `assertFails(getDocs(collection(db,'families')))` (não-admin) **e** `assertSucceeds(getDoc(doc(db,'codes',code)))` | ambas as asserções passam |
| M-12 | RT-12 / aceite: script `seed:families` existe, gera código de alta entropia e é idempotente (dry-run) | `test -f scripts/seed-families.ts && rg -n "0OoIl1" scripts/seed-families.ts` (alfabeto sem ambíguos) **e** `rg -n "codes" scripts/seed-families.ts` (checagem de colisão) | arquivo existe **e** ambas as checagens ≥1 |
| M-13 | RT/§6 / aceite: contrato `domain/types/index.ts` estendido (`phone?` em `Family`, `isChild?` em `Guest`) e deixa de ser órfão | `rg -n "phone\?" domain/types/index.ts` **e** `rg -n "isChild\?" domain/types/index.ts` **e** `rg -rn "from '.*domain/types'" lib/ app/ scripts/` | as duas primeiras ≥1 **e** a terceira ≥1 (import existe) |
| M-14 | Aceite: build limpo do projeto | `npm run build` | exit code = 0 |
| M-15 | Aceite / §9: artefato "Modelo de Planilha de Convidados" existe com as colunas exatas | `ls docs/templates/planilha-convidados.* && rg -n "id_familia.*nome_familia.*telefone_whatsapp" docs/templates/planilha-convidados.*` | arquivo existe **e** contagem ≥1 (cabeçalho com as colunas) |
| M-16 | [humano] Aceite: fluxo e2e do convidado (auth anônima, roster real, `?c=` pré-seleciona, soft gate sem bloqueio) verificado no app real | abrir `/rsvp/<código de teste>` no navegador: confirmar `auth.currentUser.isAnonymous === true`, roster e nome corretos, pré-seleção via `?c=`, navegação livre em `/presenca` e `/presentes` | verificação visual/manual marcada OK pelo operador humano |
| M-17 | [humano] §9: PREFLIGHT de banco (default vs nomeado) e seed real confirmados no Firebase Console | conferir no Firebase Console qual database tem coleções/rules e que `seed:families` (run real) criou `codes`/`families`/`guests` sem duplicação no re-run | confirmação humana no Console |

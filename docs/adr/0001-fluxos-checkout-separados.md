...
## Contexto
O domínio possui duas principais entidades de arrecadação: "Presentes" (lista tradicional ou Carrinho da Felicidade) e "Gravata do Noivo" (experiência gamificada focada em upsell e interação isolada). O processamento de pagamentos será feito via Mercado Pago.

Nota de supersessão: esta decisão substitui a premissa anterior de outro provedor. A integração de pagamento real deve seguir Mercado Pago.
...
* **O Custo:** Teremos provável duplicação no manuseio de APIs do Mercado Pago e gestão de formulários/estados de pagamento. Este custo de engenharia foi considerado aceitável em prol da taxa de conversão / UX.
